    ...
    @if ($order)
        <div class="space-y-6">
            <div class="flex justify-between items-center">
                ...
                @if($order->invoice)
                    <button wire:click="toggleInvoice"
        ...
        updateProgressBar() {
            ...
            this.progressBarWidth = statusMap[this.currentStatus] || 10;
        }
    }"
                class="mt-4"
            >
                ...
                <div class="relative pt-1">
                    <div class="flex mb-2 items-center justify-between">
                        <div class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full"
                             :class="{
                    'text-green-600 bg-green-200': currentStatus === 'delivered',
...
<script>
    ...
    function updateProgressBar(status) {
        const bar = document.querySelector('[x-ref="progressBarFill"]');
        if (!bar) return;

        let width = 10;
        if (status === 'processing') width = 40;
        else if (status === 'shipped') width = 65;
